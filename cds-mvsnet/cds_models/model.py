...
from cds_models.module import depth_regression, conf_regression, FeatureNet, CostRegNet, Refinement, get_depth_range_samples, ConvBnReLU
from cds_models.utils.warping import homo_warping_3D
from cds_models.dynamic_conv import compute_Fmatrix, compute_epipole, compute_epipole_safe
...
class CDSMVSNet(nn.Module):
    ...
    def forward(self, imgs, proj_matrices, depth_values, gt_depths=None, temperature=0.001, safe=False):
        ...
        for src_img, src_proj in zip(src_imgs, src_projs):  #imgs shape (B, N, C, H, W)
            ...
            if safe:
                ref_epipole = compute_epipole_safe(fundamental_matrix)
                src_epipole = compute_epipole_safe(torch.transpose(fundamental_matrix, 1, 2))
            else:
